    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                                ...
                                @if($post->tags->count() > 0)
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($post->tags as $postTag)
                                            <a href="{{ route('tags.show', $postTag->slug) }}" class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800 hover:bg-gray-200 transition-colors {{ $postTag->id === $tag->id ? 'ring-2 ring-indigo-500' : '' }}">
                                                #{{ $postTag->name }}
                                            </a>
                                        @endforeach
                                    </div>
        ...
        @else
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
                <p class="text-gray-600 text-lg">No posts with this tag yet. Check back soon!</p>
            </div>
        @endif
    </div>
</x-public-layout>
